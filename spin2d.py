...
for i in range(N):
    for j in range(N):
        ...
        energy[0] += spins[i, j] * (
                    up[i, j] * up_neighbour + down[i, j] * down_neighbour + left[i, j] * left_neighbour + right[
                i, j] * right_neighbour)
...
def energy_change(spin_site, bt, s_array, up_array, down_array, left_array, right_array):
    ...
    dE_tmp = 2 * s_array[i, j] * (
                up_array[i, j] * up_neighbour + down_array[i, j] * down_neighbour + left_array[i, j] * left_neighbour +
                right_array[i, j] * right_neighbour)
    return dE_tmp
