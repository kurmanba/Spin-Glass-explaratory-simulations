...
def MCstep_jit(N, L, T, h, state, acceptedMoves, energy, magnetization):
    ...
    for k in range(N):
        ...
        dE = 2 * state[i, j] * (state[(i + 1) % L, j] + state[i - 1, j] + state[i, (j + 1) % L] + state[i, j - 1]) + h * \
             state[i, j]
...
def raster_MCstep_jit(N, L, T, h, state, acceptedMoves, energy, magnetization):
    ...
    for i in range(L):
        for j in range(L):
            ...
            dE = 2 * state[i, j] * (
                        state[(i + 1) % L, j] + state[(i + L1) % L, j] + state[i, (j + 1) % L] + state[i, (j + L1) % L])
...
class Ising2D(object):
    ...
    def monteCarloStep(self):
        ...
        state, acceptedMoves, energy, magnetization = MCstep_jit(N, L, T, h, state, acceptedMoves, energy,
                                                                 magnetization)
    ...
    def rasterMonteCarloStep(self):
        ...
        state, acceptedMoves, energy, magnetization = raster_MCstep_jit(N, L, w, state, acceptedMoves, energy,
                                                                        magnetization)
...
model = Ising2D(temperature=9.88, L=50)   # Tc = 2.3
figure(1)
model.plot()
